...
from .models import DiasDisponibles
...
def admin_solicitar_turno(request):
    ...
    # Obtener lista de usuarios que pueden recibir turnos
    User = get_user_model()
    usuarios = User.objects.filter(is_active=True).exclude(is_staff=True).order_by('first_name', 'last_name')
    
    # Obtener días disponibles para el calendario
...
@login_required
def admin_configurar_dias(request):
    # Verificar si el usuario es administrador
    if not request.user.is_staff:
        return redirect('inicio')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Si es un nuevo día disponible, crear horarios predeterminados
    ...
    return render(request, 'admin/dias_changelist.html', {
        'dias': dias
    })
...
def horarios_disponibles(request, fecha):
    """
    Vista para obtener los horarios disponibles para una fecha específica
    ...
    try:
        ...
        # Buscar el día disponible
        dia = get_object_or_404(DiasDisponibles, fecha=fecha_obj, disponible=True)
        
        # Obtener horarios disponibles
        horarios = HorarioDisponible.objects.filter(dia=dia, disponible=True)
